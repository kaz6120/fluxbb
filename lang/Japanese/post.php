<?php

// Language definitions used in post.php and edit.php
$lang_post = array(

// Post validation stuff (many are similiar to those in edit.php)
'No subject'		=>	'タイトルを入力してください.',
'Too long subject'	=>	'タイトルは、７０文字（半角）以下で入力してください。',
'No message'		=>	'メッセージを入力してください。',
'Too long message'	=>	'投稿は、%s バイト以下で入力してください。',
'All caps subject'	=>	'大文字のみのタイトルは入力できません。',
'All caps message'	=>	'大文字のみのメッセージは入力できません。',
'Empty after strip'	=>	'空のBBCodeだけのメッセージは投稿できません。この状態は[quote]タグが入れ子の最大数を超えている場合などに起こります。',

// Posting
'Post errors'		=>	'投稿エラー',
'Post errors info'	=>	'下記のエラーを修正してください:',
'Post preview'		=>	'プレビュー',
'Guest name'		=>	'名前', // For guests (instead of Username)
'Post redirect'		=>	'投稿を受け付けました。リダイレクト中 …',
'Post a reply'		=>	'返信',
'Post new topic'	=>	'新しいトピック',
'Hide smilies'		=>	'スマイリーをアイコン化しない',
'Subscribe'			=>	'このトピックを購読',
'Stay subscribed'	=>	'このトピックの購読を維持',
'Topic review'		=>	'トピックレビュー（最新表示）',
'Flood start'		=>	'前の投稿から少なくとも',
'flood end'			=>	'秒は間を開けて投稿してください。少し待ってから試行してください。',
'Preview'			=>	'プレビュー', // submit button to preview message
'Flash error'		=>	'FLASH BBCODEの編集: [flash=width,height]SWFファイルのURL[/flash]',

// Edit post
'Edit post legend'	=>	'投稿を編集して送信ボタンを押してください。',
'Silent edit'		=>	'編集者を隠す（トピックビューに編集者が表示されません）',
'Edit post'			=>	'投稿の編集',
'Edit redirect'		=>	'投稿はアップデートされました。リダイレクト中 …'

);
