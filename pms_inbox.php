...
// Page ?
$p = (!isset($_REQUEST['p']) || $_REQUEST['p'] <= '1') ? '1' : intval($_REQUEST['p']);
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul id="crumbs-top">
			...
			<li><strong><?php echo $lang_pms['Multidelete'] ?></strong></li>
		</ul>
...
<div class="linkst">
	<div class="inbox crumbsplus">
		<ul id="crumbs-top">
			<li><a href="index.php"><?php echo $lang_common['Index'] ?></a></li>
			<li><a href="pms_inbox.php"><?php echo $lang_pms['Private Messages'] ?></a></li>
			<li><strong><?php echo $lang_pms['Inbox'] ?></strong></li>
		</ul>
		<div class="pagepost"></div>
		<div class="clearer"></div>
	</div>
...
// -->
	...
	<div class="block">
		<div class="pagepost">
			<p class="pagelink conl"><span class="pages-label"><?php echo $lang_common['Pages'].' '.paginate($num_pages, $p, 'pms_inbox.php?') ?></span></p>
	<?php
	// Boxes status
	if ($pun_user['g_pm_limit'] != '0' && !$pun_user['is_admmod'])
	{	
		if ($pm_boxes_empty)
			echo '<p class="postlink actions conr"><span>'.$lang_pms['Empty boxes'].'</span></p>';
		elseif ($pm_boxes_full)
			echo '<p class="postlink actions conr"><span>'.$lang_pms['Full boxes'].'</span></p>';
		else
			echo '<p class="postlink actions conr"><span>'.sprintf($lang_pms['Full to'],$per_cent_box.'%').'</span></p>';
	}
	?>
		</div>
	<form method="post" action="pms_inbox.php">	
...
?>
	<tr class="<?php echo $item_status ?>">
		...
		<td class="tc2">
		<?php
		if ($pun_user['g_view_users'] == '1') // As we are registred, is it really necessary?
		{
		...
		<?php
			if ($pun_user['g_view_users'] == '1') // As we are registred, is it really necessary?
			{
				...
				for($i = '0'; $i < count($ids_list); $i++){ // Let's make a clean receivers' list - Thanks to orklah (http://www.siteduzero.com/forum-83-548746-p1-probleme-de-foreach.html)
