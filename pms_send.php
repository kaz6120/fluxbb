...
if (isset($_POST['form_sent'])) // The post button has been pressed
{
	...
	{
		// Get receiver infos
		$result_username = $db->query('SELECT u.id, u.username, u.email, u.notify_pm, u.use_pm, u.num_pms, g.g_id, g.g_pm_limit, c.allow_msg FROM '.$db->prefix.'users AS u INNER JOIN '.$db->prefix.'groups AS g ON u.group_id=g.g_id LEFT JOIN '.$db->prefix.'messages AS pm ON pm.owner=u.id LEFT JOIN '.$db->prefix.'contacts AS c ON (c.user_id=u.id AND c.contact_id='.$pun_user['id'].') WHERE u.id!=1 AND u.username=\''.$db->escape($destinataire).'\' GROUP BY u.username') or error('Unable to get user ID.', __FILE__, __LINE__, $db->error());
		// List users infos
	...
	<form method="post" id="post" action="pms_send.php" onsubmit="return process_form(this)">
		...
		<fieldset>
			...
			<div class="infldset txtarea">
        		...
        		<?php endif; ?>
				<label class="required"><strong><?php echo $lang_common['Message'] ?> <span><?php echo $lang_common['Required'] ?></span></strong></label>
